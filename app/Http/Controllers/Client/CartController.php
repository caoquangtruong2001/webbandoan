...
class CartController extends Controller
{
    ...
    public function create(Request $request, $productId = "")
    {
        ...
        if ($request->getMethod() == 'GET') {
            if (!$product = $this->product->find($productId)) {
                ...
                $qtyp = $product->quantity;
                if($qty > $product->quantity){
                    hwa_notify_error("Sản phẩm chỉ còn ".$product->quantity ." sản phẩm");
    ...
    public function update(Request $request)
    {
        ...
        foreach ($product_ids as $product_id) {
            ...
            if ($product_id == $cart['product_id']) {
                ...
                if($qty[$cart['id']] > $product->quantity) {
                    hwa_notify_error("Số lượng sản phẩm trong kho không đủ.");
                    return redirect()->back();
                }
    ...
    public function checkout(Request $request)
    {
        ...
        if ($request->getMethod() == 'GET') {
            ...
            if($request['select-address'] == null){
                hwa_notify_error("Vui lòng chọn chi nhánh giao hàng.");
                return redirect()->route('client.checkout');

            }
            if($request['customer_address_id'] == null){
                hwa_notify_error("Vui lòng chọn địa chỉ giao hàng.");
                return redirect()->route('client.checkout');
            }
            $data_order = [
                'customer_id' => $customerId,
